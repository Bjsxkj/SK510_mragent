...
rule("swig1.base")
    on_load(function (target)
        ...
        local find_user_outdir = import("build_module_file1").find_user_outdir
        local moduletype = target:extraconf("rules", "swig1.c", "moduletype") or target:extraconf("rules", "swig1.cpp", "moduletype")

        if moduletype == "python" then
            target:set("prefixname", "_")
            local soabi = target:extraconf("rules", "swig1.c", "soabi") or target:extraconf("rules", "swig1.cpp", "soabi")
            if soabi then
        ...
        elseif moduletype == "csharp" then
            ...
            raise(vformat("unknown swig module type %s, please use `add_rules(\"swig.c\", {moduletype = \"python\"})` to set it!", moduletype))
        end
        local scriptfiles = {}
        for _, sourcebatch in pairs(target:sourcebatches()) do
            if sourcebatch.rulename:startswith("swig1.") then
...
rule("swig1.c")
    set_extensions(".i")
    add_deps("swig1.base", "c.build")
    on_buildcmd_file(function (target, batchcmds, sourcefile, opt)
        import("build_module_file1").swig_build_cmd(target, batchcmds, sourcefile, table.join({sourcekind = "cc"}, opt))
    end)
    on_build_file(function (target, sourcefile, opt)
        import("build_module_file1").swig_build_file(target, sourcefile, table.join({sourcekind = "cc"}, opt))
    end)
...
rule("swig1.cpp")
    ...
    add_deps("swig1.base", "c++.build")
    on_buildcmd_file(function (target, batchcmds, sourcefile, opt)
        -- cprint("${red}build swig c++")
        import("build_module_file1").swig_build_cmd(target, batchcmds, sourcefile, table.join({sourcekind = "cxx"}, opt))
    end)
    on_build_file(function (target, sourcefile, opt)
        -- cprint("${red}build swig c++1")
        import("build_module_file1").swig_build_file(target, sourcefile, table.join({sourcekind = "cxx"}, opt))
    end)
