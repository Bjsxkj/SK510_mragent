...
namespace imimr
{
    ...
    void IImiBaseDevice::recvLoop()
    {
        ...
        {
            while ((isGettingProperty || isAuthenticating) && isCmdClientRunning)
            {
            ...
            try
            {
                ...
                if (result)
                {
                    ...
                    if (msg_str == "heartbeat exit")
                    {
                        ...
                        lastFrameNum = 0;
                        frameNum = 0;
                    } else if (msg_str == "mrmanager error"){
                        mrManagerStatus = MrManagerStatus::ERROR;
                        if (exceptionHandler)
                        {
                            exceptionHandler->onExceptionHandle(MrAgentException::IMI_MRMANAGER_ERROR,msg_str);
                        }
                        else
                        {
                            LOGI("exceptionHandler is null");
    ...
    void IImiBaseDevice::notifyBlock() {
    ...
    void IImiBaseDevice::setMrManagerNormalStatus()  {
        if (mrManagerStatus == MrManagerStatus::EXIT) {
            if (exceptionHandler)
            {
                exceptionHandler->onExceptionHandle(MrAgentException::IMI_MRMANAGER_RESUME,"");
            }
            else
            {
                LOGI("exceptionHandler is null");
    ...
    {
        if (isGettingProperty || isAuthenticating)
        {
            return -1;
        }
    ...
    int IImiColorSensor::start(int mode)
    {
        ...
        while (retry < 5)
        {
            //            LOGI("communicationWay 5 = %s",gServerConfig.communicationWay.c_str());
            if (gServerConfig.communicationWay != "")
            {
                break;
            }
            std::this_thread::sleep_for(std::chrono::milliseconds(500));
        ...
        if (retry >= 5)
        {
            gServerConfig.communicationWay = "sharedmemory";
        }
    ...
    ImiBaseFrame IImiColorSensor::readFrame(int timeout)
    {
        ...
        if (gServerConfig.communicationWay.find("sharedmemory") != std::string::npos)
        {
            ...
            else if (bytesRead < 0)
            {
                ...
                if (color_pool.attach("COLOR"))
                {
                    EXECUTE_EVERY_N_SECONDS({LOGI("IImiColorSensor::readFrame COLOR attach completed successfully");}, 5);
                    if (jpegDecoder.Init(this->mode) != 0)
                    {
    ...
    uint64_t IImiColorFrame::getDataPointer() const
    {
        if (_colorData != nullptr)
        {
            return (uint64_t)_colorData;
        }
        else
        {
            return (uint64_t)extraData;
        }
    }
...
//            LOGI("dur = %ld",dur);
        ...
        // 重连
        if (getMrManagerStatus() == MrManagerStatus::EXIT || getMrManagerStatus() == MrManagerStatus::ERROR) {
            auto dur = now - lastReadTime;
//            LOGI("dur = %ld",dur);
            if (dur > 2000) {
                lastReadTime = now;
         ...
         int read_frameNum = 0;
         int read_Foreground_frameNum = 0;
         int read_Skeleton_frameNum = 0;
         int64_t recored_time = 0;
         int cur_frameNum = 0;
         uint32_t frameNum_Skeleton_cache = 0;
        ...
        else if (read_frameNum == 1000)
        {
            ...
            if (0 != cur_frameNum)
            {
                LOGI("usertrack read %d frames cur_frameNum =%d, read Foreground frames =%d, read Skeleton frames "
                     "=%d,fps "
                     "=%f",
                     read_frameNum, cur_frameNum, read_Foreground_frameNum, read_Skeleton_frameNum, fps);

                printf(
                    "usertrack read %d frames cur_frameNum =%d, read_Foreground_frameNum =%d, read_Skeleton_frameNum "
                    "=%d,fps =%f\n",
                    read_frameNum, cur_frameNum, read_Foreground_frameNum, read_Skeleton_frameNum, fps);
            }
        ...
        if (gServerConfig.communicationWay.find("sharedmemory") != std::string::npos)
        {
            ...
            if (foregroundbytesRead < 0 || SkeletonbytesRead < 0)
            {
                ...
                if (foreground_pool.attach("FOREGROUND") && skeleton_pool.attach("SKELETON"))
                {
                    EXECUTE_EVERY_N_SECONDS({LOGI("IImiUserTrackSensor::readFrame reattach completed successfully");}, 5);
                }
                else
                {
                    foreground_pool.release();
            ...
            frameNum = usertrackFrame->getFrameNum();
            blockProcess(now);
    ...
    void IImiUserTrackSensor::blockEvent(long now) {
        sameFrameNum ++;
        if (sameFrameNum > 100) {
            if (lastReadTime != 0L) {
                auto dur = now - lastReadTime;
//                        LOGI("dur = %ld" , dur);
    ...
    void IImiUserTrackSensor::blockProcess(long now) {
    ...
    std::string IImiBaseDevice::authenticate(const std::string &uuid, const std::string &appkey,
                                             const std::string &algorithmVersion, const std::string licenseFile)
    {
        if (isGettingProperty || isAuthenticating)
        {
            return "license验证忙碌";
        ...
        cmd.type = CMD_DO_AUTH;
        strncpy(cmd.uuid, uuid.c_str(), sizeof(cmd.uuid));
        strncpy(cmd.appKey, appkey.c_str(), sizeof(cmd.appKey));
        strncpy(cmd.algorithmVersion, algorithmVersion.c_str(), sizeof(cmd.algorithmVersion));
        LOGI("IImiBaseDevice::authenticate licenseFile %s", licenseFile.c_str());
        std::ifstream in(licenseFile, ios::binary);
        if (!in)
        {
            LOGE("IImiBaseDevice::authenticate licenseFile无效");
            return "license验证失败";
        }
        in.seekg(0, ios::end);
        ...
        in.read(reinterpret_cast<char *>(input.data()), input_size);
        in.close();
        memcpy(cmd.license, input.data(), sizeof(cmd.license));
        std::lock_guard<std::mutex> lock(send_mutex);
        if (cmd_client_socket == nullptr || cmd_client_socket.handle() == nullptr)
        {
            LOGE("IImiBaseDevice::authenticate cmd_client_socket无效");
        ...
        try
        {
            ...
            LOGI("IImiBaseDevice::authenticate ready send authenticate request");
            if (!cmd_client_socket.connected())
            {
				LOGE("IImiBaseDevice::authenticate cmd_client_socket not connected");
                isAuthenticating = false;
                return "license验证失败";
            }
            if (!cmd_client_socket.send(request, zmq::send_flags::none))
            {
				LOGE("IImiBaseDevice::authenticate send authenticate request failed");
                isAuthenticating = false;
                return "license验证失败";
            }
			LOGI("IImiBaseDevice::authenticate send authenticate request success");
        }
        catch (const zmq::error_t &e)
        {
            LOGE("IImiBaseDevice::authenticate 发送认证请求失败 : %s", e.what());
            isAuthenticating = false;
            return "license验证失败";
        }
        catch (const std::exception &e)
        ...
        try
        {
            ...
            while (index < 5)
            {
                ...
                {
                    uint16_t *cmdType = static_cast<uint16_t *>(reply.data());
                    if (cmdType != nullptr && *cmdType == auth_cmd_type::CMD_DO_AUTH_ACK)
                    {
                        cmd_data_auth_resp *resp = static_cast<cmd_data_auth_resp *>(reply.data());
                        if (strnlen(resp->result, sizeof(resp->result)) > 0)
                        {
                            LOGI("IImiBaseDevice::authenticate 认证结果: %s", resp->result);
                            isAuthenticating = false;
                            return resp->result;
                        }
                        isAuthenticating = false;
        ...
        {
            LOGE("IImiBaseDevice::authenticate 接收消息时出错: %s", e.what());
            isAuthenticating = false;
        }
        catch (const std::exception &e)
        {
