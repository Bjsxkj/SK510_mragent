...
#    include <turbojpeg.h>
#else
// Windows平台包含TurboJPEG头文件
#    include "libjpegturbo/include/turbojpeg.h"
#endif
...
#else
            // Windows平台只需要TurboJPEG实例
            tjhandle tjInstance{nullptr};
#endif
        public:
            virtual ~JpegHwDecoder()
            {
                DeInit();
            }
            ...
            int GetMaxInputDataSize() const
            {
                return m_width * m_height * 3;
            }
            void SetInputDataSize(int size)
            {
                m_input_data_size = size;
            }
            ...
            int GetMaxOutputDataSize() const
            {
                return m_width * m_height * 3;
            }
