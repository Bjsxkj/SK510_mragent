...
namespace imimr
{
    ...
    int64_t JpegHwDecoder::Init(int mode)
    {
        ...
        if (m_hw_decode)
        {
            ...
            MppBufferInfo commit = {
                .fd = m_dma_fd, .ptr = m_virt_addr, .size = m_frame_size, .type = MPP_BUFFER_TYPE_ION};
        ...
        if (tjInstance == nullptr)
        {
            LOGE("JpegHwDecoder::Init tjInitDecompress failed");
            return -1;
        }
        ...
        m_initialized.store(true, std::memory_order_release);
        LOGI("JpegHwDecoder::Init finish, mode %d, width %d, height %d", m_mode, m_width, m_height);
        return 0;
#endif
    }
    ...
    {
#ifdef ANDROID
        if (m_initialized.load(std::memory_order_acquire) == false)
        {
            return 0;
        }
        ...
        m_initialized.store(false, std::memory_order_release);
        LOGI("JpegHwDecoder::DeInit finish");
        return 0;
#endif
    }
    ...
    int64_t JpegHwDecoder::Decode(int width, int height)
    {
        ...
        {
            mpp_err("JpegHwDecoder::Decode width or height mismatch, expected %dx%d, got %dx%d", m_width, m_height,
                    width, height);
            return -1;
        }
        if (m_hw_decode)
        {
            ...
            if (meta != nullptr)
            {
                mpp_meta_set_frame(meta, KEY_OUTPUT_FRAME, m_frame);
            }
            // mpp_log("JpegHwDecoder::Decode packet size %d, packet data %p", mpp_packet_get_length(packet),
            // mpp_packet_get_data(packet));
            // 4. 解码
            MPP_RET ret = MPP_OK;
            ...
            m_output_data_size =
                m_output_data_size > m_width * m_height * 3 ? m_width * m_height * 3 : m_output_data_size;
            ret = m_mpi->reset(m_ctx);
            if (ret != MPP_OK)
            {
                mpp_err("JpegHwDecoder::Decode reset decoder failed");
            }
            // mpp_log("JpegHwDecoder::Decode decode jpeg completed successfully");
            mpp_packet_deinit(&packet);
            // static int count = 0;
            // static uint64_t diff = 0;
            // diff += std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::system_clock::now() -
            // entry_time).count(); if (count++ >= 499) {
        ...
        else
        {
            ...
            if (tjDecompressHeader3(tjInstance, (const unsigned char *)m_input_data, m_input_data_size, &width, &height,
            ...
            if (tjDecompress2(tjInstance, (const unsigned char *)m_input_data, m_input_data_size,
                              (unsigned char *)m_output_data, width, pitch, height, TJPF_RGB, 0) < 0)
            {
        ...
        if (m_initialized.load(std::memory_order_acquire) == false)
        {
            LOGE("JpegHwDecoder::Decode not initialized");
            return -1;
        }

        if (width <= 0 || width > IMG_MAX_WIDTH || height <= 0 || height > IMG_MAX_HEIGHT)
        {
            LOGE("JpegHwDecoder::Decode invalid width or height %dx%d", width, height);
            return -1;
        }

        if (width != m_width || height != m_height)
        {
            LOGE("JpegHwDecoder::Decode width or height mismatch, expected %dx%d, got %dx%d", m_width, m_height, width,
                 height);
            return -1;
        }
        ...
        if (m_input_data == nullptr)
        {
            LOGE("JpegHwDecoder::Decode input buffer is null");
            return -1;
        }

        // 解析JPEG头，获取图像尺寸和格式
        int jpegSubsamp = 0, jpegColorspace = 0;
        // 添加特殊处理：去除JPEG数据中多余的3个字节
        memcpy((uint8_t *)m_input_data + 597, (uint8_t *)m_input_data + 600, m_input_data_size - 600);
        if (tjDecompressHeader3(tjInstance, (const unsigned char *)m_input_data, m_input_data_size, &width, &height,
                                &jpegSubsamp, &jpegColorspace) < 0)
        {
            LOGE("JpegHwDecoder::Decode tjDecompressHeader3 failed: %s", tjGetErrorStr());
            return -1;
        }
        ...
        // 确保输出缓冲区足够大
        if (m_output_data_size > m_output_buffer.size())
        {
            m_output_buffer.resize(m_output_data_size);
            m_output_data = &m_output_buffer[0];
        }
        ...
        if (tjDecompress2(tjInstance, (const unsigned char *)m_input_data, m_input_data_size,
                          (unsigned char *)m_output_data, width, pitch, height, TJPF_RGB, 0) < 0)
        {
            LOGE("JpegHwDecoder::Decode tjDecompress2 failed: %s", tjGetErrorStr());
            return -1;
        }
