...
target('mragent_cpp')
    ...
    if is_plat('android') then
        ...
        add_linkdirs("cpp/common/libjpegturbo/lib/android")
        add_links("mpp")
        add_links("osal")
    ...
    if is_plat('windows') then
        ...
        -- 添加Windows环境下libjpegturbo库配置
        add_linkdirs('cpp\\common\\libjpegturbo\\lib\\win')
        add_links('turbojpeg')
        --add_defines('NOTGAME')
    end
    ...
    after_build(function(target)
        ...
        os.cp(path.join(os.scriptdir(), 'mragent.toml'), path.join(target:targetdir(), 'mragent.toml'))
         ...
         -- 添加拷贝 turbojpeg.dll 的代码
        if is_plat('windows') then
            print('==================================================================================copying turbojpeg.dll to target dir')
            local dll_source = path.join(os.scriptdir(), 'cpp', 'common', 'libjpegturbo', 'lib', 'win', 'turbojpeg.dll')
            os.cp(dll_source, path.join(target:targetdir(), 'turbojpeg.dll'))
        end
    end)
target_end()
