...
// 显式支持 uint64_t 到 C# ulong 的映射
%typemap(ctype) uint64_t "unsigned long long"
%typemap(imtype) uint64_t "ulong"
%typemap(cstype) uint64_t "ulong"
%typemap(out) uint64_t {
    $result = (unsigned long long)$1;
}
%typemap(csout) uint64_t {
...
%template(IImiImuDataProperty) imimr::IBaseProperty<imimr::ImiSkImuData>;
