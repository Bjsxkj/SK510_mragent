...
bool SharedMemoryPool::attach(const char *name) {
    ...
    EXECUTE_EVERY_N_SECONDS({LOGI("MRAgent::SharedMemoryPool::attach [[%s]] total_size %lu", shm_name.c_str(), total_size);}, 5);
    int sock_fd = socket(AF_UNIX, SOCK_STREAM, 0);
    if (sock_fd < 0) {
        EXECUTE_EVERY_N_SECONDS({LOGE("MRAgent::SharedMemoryPool::attach [[%s]] socket failed: %d", shm_name.c_str(), errno);}, 5);
        return false;
    }
    {
        struct sockaddr_un addr;
        ...
        EXECUTE_EVERY_N_SECONDS({LOGI("MRAgent::SharedMemoryPool::attach connect ret=%d, sock_fd=%d, errno=%d", ret, sock_fd, errno);}, 5);
        if (ret != 0) {
            EXECUTE_EVERY_N_SECONDS({LOGE("MRAgent::SharedMemoryPool::attach [[%s]] connect failed: %d", shm_name.c_str(), errno);}, 5);
            close(sock_fd);
            return false;
        }
    }
    ...
    if (n <= 0) {
        EXECUTE_EVERY_N_SECONDS({LOGE("MRAgent::SharedMemoryPool::attach [[%s]] recvmsg failed: %d", shm_name.c_str(), errno);}, 5);
        close(sock_fd);
        return false;
    }
    struct cmsghdr *cmsg = CMSG_FIRSTHDR(&msg);
    if (cmsg && cmsg->cmsg_len == CMSG_LEN(sizeof(int)) && cmsg->cmsg_level == SOL_SOCKET && cmsg->cmsg_type == SCM_RIGHTS) {
        ...
        EXECUTE_EVERY_N_SECONDS({LOGI("MRAgent::SharedMemoryPool::attach [[%s]] received shm_fd=%d", shm_name.c_str(), shm_fd);}, 5);
    } else {
        EXECUTE_EVERY_N_SECONDS({LOGE("MRAgent::SharedMemoryPool::attach [[%s]] invalid fd received", shm_name.c_str());}, 5);
        close(sock_fd);
        return false;
    }
    ...
    void *addr = mmap(nullptr, total_size, PROT_READ | PROT_WRITE, MAP_SHARED, shm_fd, 0);
    if (addr == MAP_FAILED) {
        EXECUTE_EVERY_N_SECONDS({LOGE("MRAgent::SharedMemoryPool::attach [[%s]] failed to mmap shared memory: %d", shm_name.c_str(), errno);}, 5);
        close(shm_fd);
        shm_fd = -1;
        return false;
    }
    m_ringbuffer = (MemoryPoolIndex *)addr;
    frames = (Frame *)((uint8_t *)addr + MEMORY_POOL_INDEX_SIZE);
    EXECUTE_EVERY_N_SECONDS({LOGI("MRAgent::SharedMemoryPool::attach [[%s]] shared memory attached successfully: fd=%d, frames=%p, m_ringbuffer=%p flag=%lu", shm_name.c_str(), shm_fd, frames, m_ringbuffer, m_ringbuffer->flag);}, 5);
    initialized = true;
    return true;
#else
    return false;
