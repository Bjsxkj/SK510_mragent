...
%define map_data_buffer_ref(ctype, arr, len)
%ignore ctype;
%typemap(jni) (ctype)  "jboolean _$1, jobject"
%typemap(jtype) (ctype) "boolean _$1, java.lang.Object"
%typemap(jstype) (ctype) "java.nio.ByteBuffer"
%typemap(javain, pre="  assert ($javainput.isDirect() || $javainput.array()!=null) : \"Buffer must not be null.\";") (ctype)
"$javainput.isDirect(), $javainput.isDirect() ? $javainput : $javainput.array()"
%typemap(in) (ctype) {
    if(_$1){
        void *data =  JCALL1(GetDirectBufferAddress, jenv, $input);
