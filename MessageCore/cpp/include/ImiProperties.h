...
enum ImiDeviceProperties
{
    ...
    IMI_PROPERTY_GET_IMU_DATA     =0x871,  //get imu data for 6 float data
...
{
	imi_sdk_version sdk_version;
	uint8_t ap_sorftware_version[32];	//ģ��̼��汾��
	//imi_chip_version security_chip_version;			//��ȫоƬ�汾��
	dsp_version dsp_2280_version;		//dsp�汾��	����fw,model,algorithm,dsp0,�����Ŀ��Բ�����
};
...
typedef struct imu_state{
	uint8_t verticalState;	//0����ֱ 1:�Ǵ�ֱ
	ImiImuData imuData;		//��ǰImuλ��
}ImiImuState;

typedef struct sk_imu_data{
	float x[6];
}ImiSkImuData;
